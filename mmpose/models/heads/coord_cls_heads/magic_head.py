...
class InceptionDWConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        return  self.conv(self.bn(torch.cat(
...
class MagicHead(BaseHead):
    ...
    def __init__(
        ...
        self.kld_loss_module = MODELS.build(loss)
        self.joint_bone_loss_module = MODELS.build(dict(type='JointBoneLoss',joint_num=out_channels))
        ...
        self.final_layer = InceptionDWConv2d(
            in_channels,
            out_channels)
    ...
    def loss(
        ...
        losses = dict()
         # calculate kld_loss
        kld_loss = self.kld_loss_module(pred_simcc, gt_simcc, keypoint_weights)
        batch_data_samples_pre_keypoints= self.decode(pred_simcc)
        batch_data_samples_gt_keypoints = self.decode(gt_simcc)

        batch_data_samples_pre_keypoints_list=[]
        batch_data_samples_gt_keypoints_list=[]
        for d in batch_data_samples_pre_keypoints:
            batch_data_samples_pre_keypoints_list.append(d.keypoints[0]) 
        for d in batch_data_samples_gt_keypoints:
            batch_data_samples_gt_keypoints_list.append(d.keypoints[0]) 

        joint_bone_loss = self.joint_bone_loss_module(torch.tensor(batch_data_samples_pre_keypoints_list),torch.tensor(batch_data_samples_gt_keypoints_list))

        #loss = kld_loss + 0.0000000005 * joint_bone_loss
        loss = 0.995*kld_loss + 0.005 * joint_bone_loss
